...
def init_session_state():
    defaults = {
        ...
        'corner_marks': DEFAULT_CORNER_MARKS,
...
# ------------------------------------------------------------
# ОБРАБОТЧИК ДАННЫХ (геодезические улучшения)
# ------------------------------------------------------------
class DataProcessor:
    @staticmethod
    ...
    def calculate_displacement(df_incl_filtered: pd.DataFrame, cycle: str, floors: List[int],
        ...
        for floor, alpha_x, alpha_y in floor_data:
            ...
            for i in range(steps):
                ...
                dx = step_height * np.tan(np.radians(alpha_x_interp))
                dy = step_height * np.tan(np.radians(alpha_y_interp))
                cum_x += dx
                cum_y += dy
...
# ------------------------------------------------------------
# ВИЗУАЛИЗАТОР (с горизонтальным масштабом)
# ------------------------------------------------------------
class Visualizer:
    @staticmethod
    def plot_3d(points, top_x, top_y, top_z, max_z,
        ...
        xs = [p[0] for p in points_scaled]
        ys = [p[1] for p in points_scaled]
        zs = [p[2] for p in points_scaled]
        fig.add_trace(go.Scatter3d(
            x=xs, y=ys, z=zs,
            mode='lines+markers',
            line=dict(color='red', width=6),
            marker=dict(size=10, color='red', symbol='circle'),
            name=f'Деформированная ось (масштаб смещений ×{horizontal_scale:.1f})'
        ))

        if not df_floors.empty:
            for i, (x, y, z) in enumerate(points_scaled[1:], start=1):
                ...
                fig.add_trace(go.Scatter3d(
                    x=[x], y=[y], z=[z],
                    mode='text',
                    text=[f"<b>{floor}эт</b><br>{dist:.3f} м (масшт.)"],
        ...
        fig.add_trace(go.Scatter3d(
            x=[0, top_x_scaled], y=[0, top_y_scaled], z=[0, top_z],
            mode='lines+markers',
        ...
        fig.add_trace(go.Scatter3d(
            x=[top_x_scaled], y=[top_y_scaled], z=[top_z],
            mode='text',
        ...
        for cx, cy in corners:
            fig.add_trace(go.Scatter3d(
                x=[cx, cx + top_x_scaled],
                y=[cy, cy + top_y_scaled],
                z=[0, top_z],
                mode='lines',
                line=dict(color='rgba(50,50,50,0.6)', width=2),
                showlegend=False
            ))
        for z_level, (x_shift, y_shift) in [(0, (0, 0)), (top_z, (top_x_scaled, top_y_scaled))]:
        ...
        fig.update_layout(
            title=f"<b>3D-модель здания – цикл {cycle}</b> (верт. масштаб {vertical_scale:.1f}, гор. масштаб {horizontal_scale:.1f})",
            scene=dict(
...
def plot_trends_with_tan(df_filtered: pd.DataFrame, floors: List[int],
    ...
    for cycle in cycles:
        ...
        for i, (x, y, z) in enumerate(points[1:], start=1):
            if i-1 < len(df_cycle_sorted):
                ...
                if floor in floors:
                    data_x[floor].append(x * horizontal_scale)
                    data_y[floor].append(y * horizontal_scale)
    ...
    fig_x.update_layout(
        title=f"Накопленные смещения по оси X (масштаб ×{horizontal_scale:.1f})",
    ...
    fig_y.update_layout(
        title=f"Накопленные смещения по оси Y (масштаб ×{horizontal_scale:.1f})",
        xaxis_title="Цикл (дата)",
        yaxis_title="Смещение Y, м",
        template="plotly_white",
        hovermode='x unified',
...
def main():
    ...
    if uploaded_file is not None:
        ...
        with st.status("Загрузка данных...", expanded=False) as status:
            ...
            if df_incl is None:
                st.stop()
            status.update(label="Данные наклономера загружены", state="complete")
        ...
        zero_cycle = st.sidebar.selectbox(
            ...
            help="Цикл, относительно которого считаются приросты углов."
        )

        st.sidebar.markdown("**Геометрия здания**")
        ...
        if filter_type != "Нет" and filter_window > len(cycles):
            st.sidebar.warning(f"Окно {filter_window} больше числа циклов ({len(cycles)}). Будет использовано min_periods=1.")

        st.sidebar.markdown("---")
        st.sidebar.subheader("🏗️ Выбор этажей и размеры")
        ...
        st.sidebar.markdown("---")
        st.sidebar.subheader("🎯 Масштаб смещений (для визуализации)")
        horizontal_scale = st.sidebar.slider(
            "Горизонтальный масштаб",
            min_value=0.5, max_value=20.0, value=st.session_state.get("horizontal_scale", 1.0), step=0.5,
            key="horizontal_scale",
            help="Увеличивает горизонтальные смещения для лучшей видимости деформаций. Не влияет на расчёты."
        )
        ...
        if "Стилобат" in all_sheets:
            ...
            corner_marks_str = st.sidebar.text_input("Марки (через запятую)", 
                                                     ...
                                                     help="Укажите 4 марки в порядке: нижний левый, нижний правый, верхний левый, верхний правый.")
            st.session_state["corner_marks"] = corner_marks_str
            marks = [int(x.strip()) for x in corner_marks_str.split(',') if x.strip()]
            if len(marks) != 4:
                st.sidebar.warning("Введите ровно 4 марки через запятую.")
            else:
                ...
                if st.sidebar.button("🔄 Пересчитать осадки", type="primary"):
                    with st.status("Расчёт осадок...", expanded=False) as status:
                        ...
                        if df_sett is not None:
                            ...
                            st.session_state["available_marks"] = available_marks
                            st.session_state["sett_calculated"] = True
                            status.update(label="Осадки рассчитаны", state="complete")
                        else:
                            status.update(label="Ошибка расчёта осадок", state="error")
                if st.session_state["df_sett_angles"] is not None:
                    df_sett = st.session_state["df_sett_angles"]
                    if df_sett is not None and not df_sett.empty:
                        st.sidebar.success(f"✅ Рассчитано {len(df_sett)} циклов.")
                        if available_marks:
                            st.sidebar.caption(f"Доступные марки: {', '.join(available_marks)}")
        else:
        ...
        horizontal_scale = st.session_state["horizontal_scale"]
        ...
        with tab_data:
            ...
            if not df_summary.empty:
                ...
                st.download_button(
                    label="📥 Скачать CSV",
                    data=csv,
                    file_name='results_all_cycles.csv',
                    mime='text/csv'
                )
            else:
